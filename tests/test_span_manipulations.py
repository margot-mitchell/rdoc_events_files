...
class TestSpanManipulations:
    ...
    def test_timestamps_in_response_time_ordered(self):
        """
        Test that each item in every list from columns with "timestamps" in the name
        appears as a value in "response_time" in the output file, with all rows 
        ordered from smallest to largest with respect to response_time.
        ...
        for input_file in span_input_files:
            ...
            # Collect all expected timestamps from columns with "timestamps" in the name
            all_expected_timestamps = []
            
            for idx, row in input_df.iterrows():
                # Find all columns with "timestamps" in the name
                timestamp_columns = [col for col in input_df.columns if 'timestamps' in col.lower()]
                
                for col in timestamp_columns:
                    # Parse the list column
            ...
            # Check that response_time values are ordered from smallest to largest
            if len(response_times) > 1:
                response_times_float = [float(t) for t in response_times]
                if response_times_float != sorted(response_times_float):
                    pytest.fail(
                        f"Output file {matching_output.name} response_time values are not ordered from smallest to largest:\n"
                        f"Current order: {response_times_float}\n"
                        f"Expected order: {sorted(response_times_float)}"
                    )

    def test_cell_order_in_grid_movement(self):
        """
        Test that for every list in the cell_order_through_grid column in the input file,
        the items of each of those lists appear in the "grid_movement" column in the 
            ...
            for idx, row in input_df.iterrows():
                ...
                if cell_order and moving_timestamps and len(cell_order) == len(moving_timestamps):
                    ...
                    for i, cell_item in enumerate(cell_order):
                        ...
                        # Check that the grid_movement column contains the cell_item
                        grid_movement_values = matching_rows['grid_movement'].astype(str).tolist()
                        if str(cell_item) not in grid_movement_values:
                            pytest.fail(
                                f"Input file {input_file.name} row {idx}: cell_order item {cell_item} not found in grid_movement column "
                                f"for response_time={moving_timestamp}. Found grid_movement values: {grid_movement_values}"
                            )
